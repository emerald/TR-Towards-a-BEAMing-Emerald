...
code are listed in Table \longref{tbl:core-erlang-optimizations}.
...
optimizations are done on generic BEAM instructions.

The optimizations performed by the Erlang/OTP compiler on BEAM
assembly, are listed in Table \longref{tbl:asm-optimizations}.
Overall, it performs function inlining, constant folding, and common
subexpression elimination.
...
1 & \texttt{beam\_block} & 153 \\
  & \multicolumn{2}{l}{\quad\emph{%
      Merge adjacent blocks; optimizations around blocks.%
    }} \\\cline{2-3}
...
3 & \texttt{beam\_jump} & 736 \\
  & \multicolumn{2}{l}{\quad\emph{%
      Remove unreachable code, duplicate code; optimize jumps.%
    }} \\\cline{2-3}
4 & \texttt{beam\_peep} & 219 \\
  & \multicolumn{2}{l}{\quad\emph{%
      Peep-hole optimizations; removing redundant tests.%
    }} \\\cline{2-3}
...
\end{tabular}
\caption{The optimizations performed on BEAM assembly by the
