...
message are specified in a functional way, it is hard to call Erlang a
``functional programming language'' --- the lifetime of an actor is
governed by the messages it receives, and these are both dynamically
typed, and arrive in an unpredictable order. Both of these
shortcomings however, can be addressed by using an optional,
additional static analyser called Dialyzer.
